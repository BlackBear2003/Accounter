...
public class LoginServiceImpl {
    ...
    @Autowired
    PwdAuthenticationProvider pwdAuthenticationProvider;
    @Autowired
    ...
    public ResponseResult loginByPassword(User user) {
        ...
        Authentication authenticate = pwdAuthenticationProvider.authenticate(usernamePasswordAuthenticationToken);

        //校验失败了
        if(Objects.isNull(authenticate)){
            return new ResponseResult(403,"用户名或密码错误！");
        }
    ...
    public ResponseResult loginByMobile(String mobile){
        ...
        Authentication authenticate = smsCodeAuthenticationProvider.authenticate(smsCodeAuthenticationToken);
